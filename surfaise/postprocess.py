...
import dolfin as df
from surfaise.common.cmd import info, parse_command_line, \
    info_split, info_yellow, info_on_red, info_cyan
from surfaise.common.cmd import mpi_is_root
import numpy as np
import os
import glob
import sys
...
def get_help(methods, methods_folder, caller=__file__, skip=0):
    ...
    methods_dir = methods_folder.split("/")[-1]
    for method in sorted(methods):
        m = __import__("{}.{}".format(methods_dir, method))
...
def get_methods(methods_folder):
    ...
    for f in glob.glob(os.path.join(methods_folder, "*.py")):
        name = f.split(methods_folder)[1][1:].split(".py")[0].split("/")[-1]
        if name[0] != "_":
...
def call_method(method, methods, scripts_folder, ts, cmd_kwargs):
    ...
    scripts_dir = scripts_folder.split("/")[-1]
    if method[-1] == "?" and method[:-1] in methods:
        m = __import__("{}.{}".format(scripts_dir,
                                      method[:-1])).__dict__[method[:-1]]
        m.description(ts, **cmd_kwargs)
    elif method in methods:
        m = __import__("{}.{}".format(scripts_dir, method)).__dict__[method]
